...
def SetM1VelocityPID(address, p, i, d, qpps):
    return _write4444(address, Cmd.SETM1PID, long(int(d * 65536)), long(int(p * 65536)), long(int(i * 65536)), qpps)


def SetM2VelocityPID(address, p, i, d, qpps):
    return _write4444(address, Cmd.SETM2PID, long(int(d * 65536)), long(int(p * 65536)), long(int(i * 65536)), qpps)
...
def SpeedM1(address, val):
    return _writeS4(address, Cmd.M1SPEED, int(val))


def SpeedM2(address, val):
    return _writeS4(address, Cmd.M2SPEED, int(val))


def SpeedM1M2(address, m1, m2):
    return _writeS4S4(address, Cmd.MIXEDSPEED, int(m1), int(m2))
...
def GetSpeedError(address):
    data = _read_n(address, Cmd.GETSPDERR, 2)
    if data[0]:
        data[1] /= 65536.0
        data[2] /= 65536.0
        return data
...
def GetPoseError(address):
    data = _read_n(address, Cmd.GETPOSERR, 2)
    if data[0]:
        data[1] /= 1024.0
        data[2] /= 1024.0
        return data
